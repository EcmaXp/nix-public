{
  ...
  outputs =
    ...
    let
      ...
      buildConfigurations =
        platform:
        (stable.nixpkgs.lib.attrsets.mapAttrs (name: value: platform (value // { hostname = name; })));
      buildPackages =
        configurations: (stable.nixpkgs.lib.attrsets.mapAttrs (name: value: value.pkgs)) configurations;
