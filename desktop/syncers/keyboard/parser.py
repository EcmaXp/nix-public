...
from .types import Assign, Equal, NotEqual, MutableShortcut, Shortcut, Token
...
class Lexer:
    def __init__(self, enum_cls: type[enum.Enum]):
        ...
        self.pattern = re.compile(
            "|".join(
                "(?P<{}>{})".format(key, re.compile(value.value).pattern)
    ...
    def lex(self, text):
        ...
        while pos < len(text):
            ...
            if not m:
                raise ValueError(f"no match: {text[pos:pos + 10]!r}")
