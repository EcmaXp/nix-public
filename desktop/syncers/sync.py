...
from .config import CONFIG_FOLDER, CONFIG_USER_FOLDER
...
def sync(
    ...
    for name, syncer in syncers.items():
        ...
        if files is not None:
            for file in files:
                if file in syncer.config_files:
                    modified |= True
...
def sync_forever(syncers: dict[str, Syncer]):
    ...
    observer = Observer()
    observer.schedule(event_handler, CONFIG_FOLDER)
    observer.schedule(event_handler, CONFIG_USER_FOLDER)
    observer.start()
