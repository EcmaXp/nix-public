...
from .keymap import MODIFIERS, PACKED_KEYS, normalize_shortcut, convert_key
...
KARABINER_GIT_CONFIG_PATH = (
    Path(__file__).parents[3] / "programs/karabiner/karabiner.json"
)
...
def get_chain_shortcuts(key: str, operation) -> tuple[str, list[Shortcut]]:
    condition_shortcuts = []
    if isinstance(operation, list):
        if is_chain_shortcuts(operation):
        ...
        if packed_keys:
            for token, new_tokens in packed_keys.items():
                for new_token in new_tokens:
                    new_key = key.replace(token, new_token)
                    new_value = value.replace(token, new_token)
                    manipulators.append(
...
def _build_manipulator_to_keys_with_programs(operations: str | dict) -> dict | None:
    ...
    if operations.startswith(("http://", "https://")):
        assert '"' not in operations
        return {"shell_command": f'open "{operations}"'}
...
def save_rules_to_git():
    ...
    with KARABINER_GIT_CONFIG_PATH.open("w") as f:
